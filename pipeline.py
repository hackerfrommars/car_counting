        ...
        for i, path in enumerate(self.pathes):
            ...
            if (
                ...
                with conn:
                    ...
                    photo_location = f"{date_today}/{time_now}.png"
                    if not os.path.exists(f"./out/{date_today}"):
                        os.makedirs(f"./out/{date_today}")
                    frame = context['frame'].copy()
                    utils.save_frame(frame, f"out/{photo_location}")
                    utils.insert_into_db(conn, photo_location=photo_location)
...
class Visualizer(PipelineProcessor):
    ...
    def draw_boxes(self, img, pathes, exit_masks=[]):
        for (i, match) in enumerate(pathes):
            ...
            cv2.putText(img, ("Width: {width}, Height: {height} ".format(width=w, height=h)), (x, y),
